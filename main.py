...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        logger.info(f"OpenAI responded to user {user.id}: '{ai_response}'")
        ...
        if user_conversations[user.id] and user_conversations[user.id][-1]["role"] == "user":
            user_conversations[user.id].pop()
...
async def send_keyword_joke(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    user_id = update.effective_user.id
    logger.info(f"User {user.id} requested a joke.")

    if user_id not in user_conversations:
        user_conversations[user.id] = [{"role": "system", "content": "You are a helpful AI assistant."}]
    ...
    try:
        ...
        logger.info(f"OpenAI generated joke for user {user.id}: '{joke}'")
        # Add the joke to history for context
        user_conversations[user.id].append({"role": "assistant", "content": joke})

    except Exception as e:
        logger.error(f"Error generating joke for user {user.id}: {e}")
